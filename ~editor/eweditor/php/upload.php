...
break;
case 3:
OutScript("parent.UploadError('UPLOAD_ERR_PARTIAL: The uploaded file was only partially uploaded!')");
break;
case 6:
OutScript("parent.UploadError('UPLOAD_ERR_NO_TMP_DIR: Missing a temporary folder!')");
break;
case 7:
OutScript("parent.UploadError('UPLOAD_ERR_CANT_WRITE: Failed to write file to disk!')");
break;
}
exit;
}
...
echo "<html><head><title>eWebEditor</title><meta http-equiv='Content-Type' content='text/html; charset=utf-8'></head><body>".
"<input type=hidden id=UploadText value=\"".HTMLEncode($sContent)."\">".
"</body></html>";
OutScript("parent.setHTML(document.getElementById('UploadText').value);try{parent.addUploadFiles('".$GLOBALS["sOriginalFileName"]."', '".$GLOBALS["sPathFileName"]."');} catch(e){} parent.remoteUploadOK();");
}
...
$s_PortSecure = $_SERVER["SERVER_PORT_SECURE"];
$s_Protocol = "http://";
if ($s_PortSecure == "1"){
$s_Protocol = "https://";
}
$sHost = $s_Protocol.$_SERVER["HTTP_HOST"];
return $sHost.$url;
}
