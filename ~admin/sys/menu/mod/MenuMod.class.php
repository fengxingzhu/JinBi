...
class MenuMod extends Model
{
	...
	{
		return $this->get_all('SELECT * FROM '.DB_MX_PRE.'menu WHERE parent_id="'.$pid.'" AND langtp="'.$user['langtp'].'" AND typeid=0 ORDER BY sort ASC');
	}
	public function getpmenu($pid)
	{
		$d = $this->get_row('SELECT path FROM '.DB_MX_PRE.'menu WHERE id="'.$pid.'"');
		if(empty($d['path'])){
			return '<a href="javascript:;" brurl="sys/menu/?app.html" class="mx_ahref">菜单频道</a>';
		}else{
			$dd = $this->get_all('SELECT title,id FROM '.DB_MX_PRE.'menu WHERE id IN('.$d['path'].') ORDER BY grade ASC');
			$res = array('<a href="javascript:;" brurl="sys/menu/?app.html" class="mx_ahref">菜单频道</a>');
			foreach($dd as $list){
				array_push($res,'<a href="javascript:;" brurl="sys/menu/?app/pid/'.$list['id'].'.html" class="mx_ahref">'.$list['title'].'</a>');
			}
			return join(' > ',$res);
		}
	}
	...
	public function edit($id)
	{
		...
		$this->update(DB_MX_PRE.'menu',array('title','sort','typeid','admurl','livsid','frturl'),'id="'.$id.'"');
	}
	//删除目录
