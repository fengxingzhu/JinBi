 ...
 * Modify By:Anson QQ:75526201 chent.tiansong@yahoo.com.cn
 */
...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 rp: 15, // 每页默认的结果数
			 rpOptions: [10,15,20,25,40,60,80,100],//可选择设定的每页结果数
			 title: false,//是否包含标题
			 pagestat: '当前页记录为{from} - {to},总记录{total}条',//显示当前页和总页面的样式
			 procmsg: '正在加载...',//正在处理的提示信息
			 query: '',//搜索查询的条件
			 qtype: '',//搜索查询的类别
			 nomsg: '无数据',//无结果的提示信息
		...
		var g = {
				...
				if (this.colresize)
					{
						...
						var fdv = $('th[axis="col'+n+'"]').attr("abbr");
						if(p.svwurl) $.post(p.svwurl,{fd:fdv,vl:nw},function(text){
							//
						});
					}
			...
			switchCol: function(cdrag,cdrop) { //switch columns
				...
				$('tbody tr',t).each
					...
					this.hDiv.scrollLeft = this.bDiv.scrollLeft;
				//保存当前顺序
				if(p.gridheadersorturl){
					var table = $(t).parent().parent(),hdsort = [];
					$('thead th',table).each(function(i,o){
						var abbrfd = $(o).attr("abbr");
						if(abbrfd && abbrfd!='id') hdsort.push(abbrfd);
					});
					if(hdsort.length>0){
						$.post(p.gridheadersorturl,{headsort:hdsort.join(",")},function(text){
							//
						});
					}
				}
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>关键字 <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select><input type='button' value='查询' /> <input type='button' value='清除' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[value=查询]',g.sDiv).click(function(){ g.doSearch(); });
				$('input[value=清除]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);				
